...
class ReportService {
  ...
  Future<_MonthData> _fetchMonthData(String businessId, int month, int year) async {
    ...
    for (final raw in breakdowns) {
      ...
      final bucket     = _classifyResidenceBucket(country: country, nationality: nationality, isOverseas: isOverseas);
  ...
  _MonthData _mergeMonthData(int month, List<_MonthData> list) {
    ...
    for (final md in list) {
      md.countryByDay.forEach((country, days) {
        countryByDay.putIfAbsent(country, () => {});
        days.forEach((day, count) { countryByDay[country]![day] = (countryByDay[country]![day] ?? 0) + count; });
      });
      md.residentsByDay.forEach((day, cats) {
        residentsByDay.putIfAbsent(day, () => {});
        cats.forEach((cat, count) { residentsByDay[day]![cat] = (residentsByDay[day]![cat] ?? 0) + count; });
      });
      md.sexByDay.forEach((day, sexMap) {
        sexByDay.putIfAbsent(day, () => {});
        sexMap.forEach((sex, cats) {
          sexByDay[day]!.putIfAbsent(sex, () => {});
          cats.forEach((cat, count) { sexByDay[day]![sex]![cat] = (sexByDay[day]![sex]![cat] ?? 0) + count; });
        });
      });
      md.roomsOccupied.forEach((day, count) { roomsOccupied[day] = (roomsOccupied[day] ?? 0) + count; });
      md.guestNightsByDay.forEach((day, count) { guestNightsByDay[day] = (guestNightsByDay[day] ?? 0) + count; });
      md.guestNightsPerArrivalDay.forEach((day, count) { guestNightsPerArrivalDay[day] = (guestNightsPerArrivalDay[day] ?? 0) + count; });
  ...
  Uint8List _buildWorkbook({
    ...
    if (opts.includeDailySheet) {
      for (int i = 0; i < businesses.length; i++) {
        _buildDailySheet(excel, businesses[i], selectedMonthPerBiz[i], params.year, daysInMonth);
      }
    }
    if (opts.includeCountrySumSheet) {
      final merged = _mergeMonthData(params.month, selectedMonthPerBiz);
      _buildCountrySummarySheet(excel, merged, totalRoomsAll, params.year, params.month, daysInMonth);
    }
  ...
  void _buildDailySheet(Excel excel, _BusinessInfo biz, _MonthData md, int year, int daysInMonth) {
    ...
    // ── Column header row (with borders) ─────────────────────────────────
    _writeDayColHeaders(sheet, row++);

    // helpers
    int cnt(String country, int day) => md.countryByDay[country.toUpperCase()]?[day] ?? 0;
    int countryTotal(int day) => md.countryByDay.values.fold<int>(0, (s, d) => s + (d[day] ?? 0));
    ...
    prVals[32] = (res(0, 'philippine_resident_filipino') + res(0, 'philippine_resident_foreign')).toString();
    _styledRow(sheet, row++, prVals, bgHex: _kGreen, bold: true);

    _blankDataRow(sheet, row++, 33); // separator
    ...
    for (final cg in kCountryRows) {
      ...
      if (isLastInSub) {
        ...
        _blankDataRow(sheet, row++, 33); // separator between sub-regions
      }
    }

    // ── OTHERS AND UNSPECIFIED — label + data on SAME row (FIX) ──────────
    _styledRow(sheet, row++,
        dayValues('OTHERS AND UNSPECIFIED NON-PHILIPPINE RESIDENCES', (d) => res(d, 'unspecified_guest')),
    ...
    nprTotal = md.countryByDay.values.fold<int>(0, (a, d) => a + (d[0] ?? 0)) + res(0, 'unspecified_guest');
    nprVals[32] = nprTotal.toString();
    _styledRow(sheet, row++, nprVals, bgHex: _kGreen, bold: true);

    // ── OVERSEAS FILIPINOS — label + data on SAME row (FIX) ──────────────
    _blankDataRow(sheet, row++, 33);
    _styledRow(sheet, row++,
    ...
    _styledRow(sheet, row++,
        dayValues('   Total Non-Philippine Residents', (d) => countryTotal(d) + res(d, 'unspecified_guest')),
        bgHex: _kGreen, bold: true);
    _styledRow(sheet, row++,
    ...
    for (int d = 1; d <= 31; d++) availVals.add(d <= daysInMonth ? biz.totalRooms.toString() : '');
    ...
    occVals.add(roomsAvail > 0 ? (totalRoomsOcc / roomsAvail * 100).toStringAsFixed(2) : '0');
    ...
    void sexSection(String gender, String label) {
      ...
      _styledRow(sheet, row++, dayValues('a. Philippine Residents', (d) =>
          sex(d, gender, 'philippine_resident_filipino') + sex(d, gender, 'philippine_resident_foreign')));
      _styledRow(sheet, row++, dayValues(
          'b. Non-Philippine/Foreign Residents (including unspecified)',
          (d) => sex(d, gender, 'foreign_resident')));
      _styledRow(sheet, row++, dayValues('c. Overseas Filipinos',       (d) => sex(d, gender, 'overseas_filipino')));
      _styledRow(sheet, row++, dayValues('d. Others/Unspecified Guest', (d) => sex(d, gender, 'unspecified_guest')));
      final totVals = dayValues('x. Total', (d) =>
          sex(d, gender, 'philippine_resident_filipino') + sex(d, gender, 'philippine_resident_foreign') +
          sex(d, gender, 'foreign_resident') + sex(d, gender, 'unspecified_guest') +
          sex(d, gender, 'overseas_filipino'));
      totVals[32] = (sex(0, gender, 'philippine_resident_filipino') + sex(0, gender, 'philippine_resident_foreign') +
          sex(0, gender, 'foreign_resident') + sex(0, gender, 'unspecified_guest') +
  ...
  void _buildCountrySummarySheet(
    ...
    _blankDataRow(sheet, row++, 2); // blank spacer

    int totCnt(String c) => md.countryByDay[c.toUpperCase()]?[0] ?? 0;
    int totRes(String c) => md.residentsByDay[0]?[c] ?? 0;
    int totCountryAll()  => md.countryByDay.values.fold<int>(0, (a, d) => a + (d[0] ?? 0));
    ...
    // ── OTHERS — label + data on SAME row (FIX) ──────────────────────────
    _styledRow(sheet, row++,
        ['OTHERS AND UNSPECIFIED NON-PHILIPPINE RESIDENCES', totRes('unspecified_guest').toString()],
        bgHex: _kBlue, bold: true);

    _styledRow(sheet, row++, ['TOTAL NON-PHILIPPINE RESIDENTS', nprTotal.toString()],
        bgHex: _kGreen, bold: true);

    // ── OVERSEAS — label + data on SAME row (FIX) ────────────────────────
    _styledRow(sheet, row++,
        ['OVERSEAS FILIPINOS*', totRes('overseas_filipino').toString()],
        bgHex: _kBlue, bold: true);

    final grandTotal = totRes('philippine_resident_filipino') + totRes('philippine_resident_foreign') +
        nprTotal + totRes('overseas_filipino');
    ...
    void sumSexRows(String gender, String label) {
      ...
      _styledRow(sheet, row++, [
        ...
        (totSex(gender, 'philippine_resident_filipino') + totSex(gender, 'philippine_resident_foreign')).toString(),
      ]);
      _styledRow(sheet, row++, [
        'b. Non-Philippine/Foreign Residents (including unspecified)',
      ...
      _styledRow(sheet, row++, [
        'x. Total',
        (totSex(gender, 'philippine_resident_filipino') + totSex(gender, 'philippine_resident_foreign') +
         totSex(gender, 'foreign_resident') + totSex(gender, 'unspecified_guest') +
  ...
  void _buildMonthlySummarySheet(
    ...
    _MonthData mdFor(int m) => allMonths.firstWhere((x) => x.month == m, orElse: () => _emptyMonth(m));
    int mCnt(String country, int m) => mdFor(m).countryByDay[country.toUpperCase()]?[0] ?? 0;
    int mRes(int m, String cat)     => mdFor(m).residentsByDay[0]?[cat] ?? 0;
    int mCountryAll(int m)          => mdFor(m).countryByDay.values.fold<int>(0, (a, d) => a + (d[0] ?? 0));
    ...
    // ── OTHERS — label + data on SAME row (FIX) ──────────────────────────
    _styledRow(sheet, row++,
        monthValues('OTHERS AND UNSPECIFIED NON-PHILIPPINE RESIDENCES', (m) => mRes(m, 'unspecified_guest')),
        bgHex: _kBlue, bold: true);

    _styledRow(sheet, row++,
        monthValues('TOTAL NON-PHILIPPINE RESIDENTS', (m) => mCountryAll(m) + mRes(m, 'unspecified_guest')),
        bgHex: _kGreen, bold: true);
    ...
    _styledRow(sheet, row++,
        ...
        bgHex: _kGreen, bold: true);
    _styledRow(sheet, row++,
        monthValues('   Total Guest with Unspecified Residence', (m) => mRes(m, 'unspecified_guest')),
        bgHex: _kGreen, bold: true);
    ...
    for (int m = 1; m <= 12; m++) {
      final md2    = mdFor(m);
      final guests = mRes(m, 'philippine_resident_filipino') + mRes(m, 'philippine_resident_foreign') +
          mCountryAll(m) + mRes(m, 'unspecified_guest') + mRes(m, 'overseas_filipino');
      totalGuestNights += md2.guestNights;
      totalGuests      += guests;
      alsCols.add(guests > 0 ? (md2.guestNights / guests).toStringAsFixed(2) : '0');
    }
    alsCols.add(totalGuests > 0 ? (totalGuestNights / totalGuests).toStringAsFixed(2) : '0');
    ...
    void mSexSection(String gender, String label) {
      ...
      _styledRow(sheet, row++, monthValues('a. Philippine Residents', (m) =>
          mSex(m, gender, 'philippine_resident_filipino') + mSex(m, gender, 'philippine_resident_foreign')));
      ...
      _styledRow(sheet, row++, monthValues('x. Total', (m) =>
          mSex(m, gender, 'philippine_resident_filipino') + mSex(m, gender, 'philippine_resident_foreign') +
          mSex(m, gender, 'foreign_resident') + mSex(m, gender, 'unspecified_guest') +
  ...
  /// Header block cell — no border.
  /// Pass [mergeToCol] to merge across columns and truly center (for title/region lines).
  void _hdrCell(
    Sheet sheet,
    int row,
    String value, {
  ...
  /// Standalone label cell (section titles inside data area, footer lines) — no border.
  void _styledCell(
  ...
  /// Section-header row — colored background + thin borders across [totalCols] columns.
  void _sectionRow(
  ...
  /// Data row — white (or colored) background + thin borders on every cell.
  void _styledRow(
  ...
  /// Blank separator row inside the data table — white background, no borders.
  /// Fills [numCols] columns so every cell in the row is explicitly white
  /// (prevents gray/unformatted cells showing through).
  ...
  /// Column-header row for the daily sheet — with thin borders.
  void _writeDayColHeaders(Sheet sheet, int row) {
    ...
    final totalCell = sheet.cell(CellIndex.indexByColumnRow(columnIndex: 32, rowIndex: row));

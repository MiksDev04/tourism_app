...
import '../../../api/admin_report_api.dart' hide GeneratedReport;
import '../widgets/report_view_modal.dart';
...
class _AdminReportsPageState extends State<AdminReportsPage> {
  ...
  Future<void> _fetchReports() async {
    ...
    try {
      ...
      setState(() {
        _reports =
            (rows as List).map((r) => GeneratedReport.fromRow(r)).toList();
      });
    } on SocketException {
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _downloadReport(GeneratedReport report) async {
    ...
    try {
      ...
      final fileData =
          await _supabase.storage.from('reports').download(filePath);
